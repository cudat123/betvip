...
function predictNextResult() {
  ...
  if (CAU_DATABASE[pattern]) {
    const predictedResult = CAU_DATABASE[pattern] === "T" ? "Tài" : "Xỉu";
    return {
      prediction: predictedResult,
      reason: `Tìm thấy pattern trong database: ${pattern} → ${predictedResult}`,
      pattern: pattern,
      confidence: 0.85
    };
  }
...
async function autoUpdate() {
  ...
  if (!sessionHistory.has(data.SessionId)) {
    ...
    if (old) {
      ...
      predictionHistory.delete(data.SessionId);
    }
  }

  const next = data.SessionId + 1;
  if (!predictionHistory.has(next)) {
    const predResult = predictNextResult();
    lastPrediction = predResult.prediction;
    lastPattern = predResult.pattern;
    predictionHistory.set(next, lastPrediction);
  }
}
...
app.get('/api/data', async (req, res) => {
  ...
  if (!lastRealData) {
    return res.json({ 
      error: "Đang chờ dữ liệu",
      timestamp: new Date().toISOString()
    });
  }

  const next = lastRealData.SessionId + 1;
  const nextPrediction = predictionHistory.get(next) || lastPrediction;
  ...
  const currentPrediction = predictionHistory.get(lastRealData.SessionId);
  if (currentPrediction) {
    ketqua_ddoan = evaluate(lastRealData.KetQua, currentPrediction);
  }
  ...
  res.json({
    ...
    thong_ke: {
      ...
      phien_moi_nhat: lastRealData.SessionId,
